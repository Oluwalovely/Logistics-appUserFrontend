...
import { MapPin, Package, Camera, Radio, Car, XCircle, CheckCircle, Trash2, User, Phone } from 'lucide-react';
        ...
        <>
            ...
            <div className="container py-4">
                <div className="row justify-content-center">
                    <div className="col-md-8">
                        ...
                        <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14 }}>
                            <div className="card-body d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <small className="text-muted">Placed on {new Date(order.createdAt).toLocaleDateString('en-GB')}</small>
                                </div>
                                    ...
                                    <div className="d-flex justify-content-between align-items-center">
                                        {statusSteps.map((step, index) => (
                                            <div key={step} className="text-center flex-fill">
                                                <div className="rounded-circle mx-auto mb-1 d-flex align-items-center justify-content-center"
                                                    style={{ width: 30, height: 30, background: index <= currentStep ? '#0d1f4f' : '#e8edf7', color: index <= currentStep ? '#fff' : '#aab0c0', fontSize: 11, fontWeight: 'bold', border: index === currentStep ? '2px solid #e8610a' : 'none' }}>
                                                    {index < currentStep ? '✓' : index + 1}
                                                </div>
                                                <small className="d-none d-md-block" style={{ fontSize: 9, color: index <= currentStep ? '#0d1f4f' : '#aab0c0', textTransform: 'uppercase' }}>
                                                    {step}
                                                </small>
                                            </div>
                                        ))}
                        ...
                        {location && ['picked-up', 'in-transit'].includes(order.status) && (
                            <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14, borderLeft: '4px solid #e8610a' }}>
                                <div className="card-body">
                                    ...
                                    <p className="mb-2 text-muted small">Updated: {new Date(location.recordedAt).toLocaleTimeString()}</p>
                                    <a href={`https://www.google.com/maps?q=${location.location.lat},${location.location.lng}`}
                        ...
                        {/* Route — now with sender/receiver details */}
                        <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14 }}>
                            <div className="card-body">
                                <h6 className="fw-bold mb-3 d-flex align-items-center gap-2" style={{ color: '#0d1f4f' }}>
                                ...
                                <div className="d-flex gap-3">
                                    ...
                                    <div className="flex-fill p-3 rounded" style={{ background: '#f4f6fb' }}>
                                        <small className="text-muted d-block mb-2" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 1 }}>Pickup</small>
                                        <p className="mb-0 fw-semibold small">{order.pickupAddress?.street}</p>
                                        <small className="text-muted">{order.pickupAddress?.city}</small>
                                        {(order.pickupAddress?.senderName || order.pickupAddress?.senderPhone) && (
                                            <div className="mt-2 pt-2" style={{ borderTop: '1px solid #e0e5f0' }}>
                                                {order.pickupAddress?.senderName && (
                                                    <div className="d-flex align-items-center gap-1 mb-1">
                                                        <User size={11} color="#94a3b8" />
                                                        <small style={{ fontSize: '0.72rem', color: '#0d1f4f', fontWeight: 600 }}>{order.pickupAddress.senderName}</small>
                                                    </div>
                                                )}
                                                {order.pickupAddress?.senderPhone && (
                                                    <div className="d-flex align-items-center gap-1">
                                                        <Phone size={11} color="#94a3b8" />
                                                        <a href={`tel:${order.pickupAddress.senderPhone}`} style={{ fontSize: '0.72rem', color: '#e8610a', fontWeight: 600, textDecoration: 'none' }}>
                                                            {order.pickupAddress.senderPhone}
                                                        </a>
                                                    </div>
                                                )}
                                    ...
                                    {/* Delivery */}
                                    <div className="flex-fill p-3 rounded" style={{ background: '#f4f6fb' }}>
                                        <small className="text-muted d-block mb-2" style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 1 }}>Delivery</small>
                                        ...
                                        {(order.deliveryAddress?.receiverName || order.deliveryAddress?.receiverPhone) && (
                                            <div className="mt-2 pt-2" style={{ borderTop: '1px solid #e0e5f0' }}>
                                                {order.deliveryAddress?.receiverName && (
                                                    <div className="d-flex align-items-center gap-1 mb-1">
                                                        <User size={11} color="#94a3b8" />
                                                        <small style={{ fontSize: '0.72rem', color: '#0d1f4f', fontWeight: 600 }}>{order.deliveryAddress.receiverName}</small>
                                                    </div>
                                                )}
                                                {order.deliveryAddress?.receiverPhone && (
                                                    <div className="d-flex align-items-center gap-1">
                                                        <Phone size={11} color="#94a3b8" />
                                                        <a href={`tel:${order.deliveryAddress.receiverPhone}`} style={{ fontSize: '0.72rem', color: '#e8610a', fontWeight: 600, textDecoration: 'none' }}>
                                                            {order.deliveryAddress.receiverPhone}
                                                        </a>
                                                    </div>
                                                )}
                            ...
                            <div className="col-md-6">
                                <div className="card border-0 shadow-sm h-100" style={{ borderRadius: 14 }}>
                                    <div className="card-body">
                                        ...
                                        <p className="mb-0 fw-bold" style={{ color: '#e8610a' }}>₦{order.price?.toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="col-md-6">
                                        ...
                                        {order.driver ? (
                                            <>
                                                ...
                                                <p className="mb-0 small">
                                                    <a href={`tel:${order.driver.phone}`} style={{ color: '#e8610a', textDecoration: 'none' }}>{order.driver.phone}</a>
                                                </p>
                                            </>
                                        ) : (
                                ...
                                <div className="card-body">
                                    ...
                                    <div className="d-flex gap-3 flex-wrap">
                                        {order.images.map((img, i) => (
                                            <img key={i} src={img.url} alt={`Package ${i + 1}`}
                                                onClick={() => setLightboxImg(img.url)}
                                                style={{ width: 120, height: 120, objectFit: 'cover', borderRadius: 10, border: '2px solid #e8edf7', cursor: 'pointer', transition: 'transform 0.2s' }}
                        ...
                        <div className="d-flex flex-column gap-2">
                            ...
                            {!['delivered', 'confirmed', 'cancelled'].includes(order.status) && (
                                <button className="btn btn-outline-danger" style={{ borderRadius: 10 }}
                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content border-0" style={{ borderRadius: 16 }}>
                        <div className="modal-body text-center p-4">
                            ...
                            <p className="text-muted small mb-4">This will permanently remove the order from your history. This cannot be undone.</p>
                            <div className="d-flex gap-2 justify-content-center">
                                <button className="btn btn-outline-secondary px-4" style={{ borderRadius: 10 }} data-bs-dismiss="modal">Go Back</button>
            ...
            {lightboxImg && (
                <div onClick={() => setLightboxImg(null)}
                    ...
                    <img src={lightboxImg} alt="Full size" style={{ maxWidth: '90vw', maxHeight: '90vh', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }} />
                </div>
            )}
        </>
    );
